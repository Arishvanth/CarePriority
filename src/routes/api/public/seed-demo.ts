      ...
      POST: async () => {
        ...
        for (const u of DEMO) {
          ...
          if (error) {
            ...
            out.push(`${u.email}: existing (${error.message})`);
          ...
          if (userId) {
            ...
            const { error: rErr } = await supabaseAdmin.from("user_roles").insert({ user_id: userId, role: u.role as never });
            if (rErr) out.push(`  role error: ${rErr.message}`);
          } else out.push(`  no user id`);
        }
